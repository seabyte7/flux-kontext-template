...
describe("payment-security", () => {
  beforeAll(() => {
    process.env.PAYMENT_VALIDATION_SECRET =
      "test_payment_validation_secret_32_chars";
  });
  ...
  describe("STANDARD_PRICING", () => {
    ...
    it("should have correct credit pack prices", () => {
      expect(STANDARD_PRICING.creditPacks.starter.price).toBe(4.9);
      expect(STANDARD_PRICING.creditPacks.starter.credits).toBe(600);
      expect(STANDARD_PRICING.creditPacks.creator.price).toBe(15.0);
      expect(STANDARD_PRICING.creditPacks.business.price).toBe(60.0);
    });
  });

  describe("mapCreemProductIdToInternal", () => {
    it("should map CREEM subscription IDs to internal IDs", () => {
      const result = mapCreemProductIdToInternal(
        "subscription",
        "FluxKontext-Plus-Monthly",
      );
